...
<p align="center">
  ...
  <a href="https://github.com/amirhasanzadehpy/Pogo/releases/tag/v0.2.2"><img src="https://img.shields.io/badge/release-v0.2.2-56e39f?style=flat-square" alt="Release v0.2.2"></a>
...
Download `pogo-0.2.2.vsix` from
[GitHub Releases](https://github.com/amirhasanzadehpy/Pogo/releases), then run:

```sh
code --install-extension "$HOME/Downloads/pogo-0.2.2.vsix"
```
...
```sh
tar -xzf pogo-v0.2.2-linux-amd64.tar.gz  # use darwin and/or arm64 when needed
...
```powershell
Expand-Archive .\pogo-v0.2.2-windows-amd64.zip -DestinationPath .\pogo
...
gh release download v0.2.2 \
  ...
  --pattern 'pogo-v0.2.2-linux-amd64.tar.gz' \
...
`shasum -a 256 pogo-v0.2.2-darwin-arm64.tar.gz` and compare it with the matching
line in `checksums.txt`. On Windows, use:

```powershell
Get-FileHash .\pogo-v0.2.2-windows-amd64.zip -Algorithm SHA256
```
