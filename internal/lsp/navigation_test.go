...
func TestDefinitionReturnsExactSchemaLocations(t *testing.T) {
	...
	tests := []struct {
		...
		{"nested Q field after annotate", "from django.db.models import Count, Q\nfrom myapp.models import Book\nBook.objects.annotate(total=Count(\"id\")).filter(Q(author__na|me=1))", protocol.Range{Start: protocol.Position{Line: 2, Character: 4}, End: protocol.Position{Line: 2, Character: 29}}},
		{"function expression field", "from django.db.models.functions import Concat\nfrom myapp.models import Book\nBook.objects.annotate(label=Concat(\"author__na|me\"))", protocol.Range{Start: protocol.Position{Line: 2, Character: 4}, End: protocol.Position{Line: 2, Character: 29}}},
		...
		{"annotated queryset relation", "from django.db.models import QuerySet\nfrom myapp.models import Book\nitems: QuerySet[Book] = get_items()\nitems.select_related(\"au|thor\")", protocol.Range{Start: protocol.Position{Line: 4, Character: 4}, End: protocol.Position{Line: 4, Character: 88}}},
		...
		{"shortcut reverse manager Q field", "from django.db.models import Q\nfrom django.shortcuts import get_object_or_404\nfrom myapp.models import Book\nbook = get_object_or_404(Book, id=1)\nbook.author.books.filter(Q(author__na|me=1))", protocol.Range{Start: protocol.Position{Line: 2, Character: 4}, End: protocol.Position{Line: 2, Character: 29}}},
